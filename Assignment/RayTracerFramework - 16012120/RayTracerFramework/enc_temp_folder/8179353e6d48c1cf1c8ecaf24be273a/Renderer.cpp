...
void Renderer::Render(const std::vector<Sphere>& spheres, const int &iteration)
{
	...
	for (unsigned y = 0; y < height; ++y) {
		for (unsigned x = 0; x < width; ++x, ++pixel) {
			...
			raydir.normalize();
			*pixel = RayTracer::GetInstance()->Trace(raydir, spheres);
		}
	}
...
void Renderer::OutputToFile(const int &iteration)
{
	...
	// std::string c1 = fileInfo.str();
	// char c[921600];
	//
	// sprintf(c, "%.921599s", c1.c_str());
...
void Renderer::SmoothScaling()
{
	...
	for (float r = 0; r <= numOfFrames; r++)
	{
		...
		for (int i = 0; i < animations.numOfObjects; i++)
		{
			...
			spheres.emplace_back(Sphere(frame.pos, frame.scale, frame.brushColour, 1, 0.0));
		}
...
void Renderer::SmoothScalingUO()
{
	...
	{
		spheres.push_back(Sphere(Vec3f(0.0, -10004, -20), 10000, Vec3f(0.20, 0.20, 0.20), 0, 0.0));
		spheres.push_back(Sphere(Vec3f(0.0, 0, -20), r / 100, Vec3f(1.00, 0.32, 0.36), 0, 0.5)); // Radius++ change here
